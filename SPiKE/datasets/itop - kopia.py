...
class ITOP(Dataset):
    ...
    def _load_data(self, use_valid_only):
        ...
        point_clouds_dict = {
            ...
            for i, identifier in enumerate(identifiers) if i < len(point_clouds)
        }
        joints_dict = {
        ...
        self.valid_identifiers = self.valid_identifiers[:467]  # Keep only first 467 frames
    ...
    def __len__(self):
        return min(len(self.valid_identifiers), 466)  # Limit to first 467 frames
    ...
    def __getitem__(self, idx):
        ...
        if joints is None or any(frame is None for frame in clip):
            #print(f"⚠️ Skipping invalid frame: {identifier}")
            return self.__getitem__((idx + 1) % len(self))  # Skip Wrap around to a valid frame
